...
| Business analytics | `GET /analytics/overview?period=today|yesterday|week|month|year|custom&start_date=YYYY-MM-DD&end_date=YYYY-MM-DD` |
...
Order Management V1 adds the `orders` ledger for actual deals and fulfilment. An order has a unique manual order number, required customer, optional opportunity and quotation links, order date, original-currency order amount, payment/production/shipping status, delivery dates, notes, and the manual RMB inputs `rmb_received_amount`, `purchase_cost`, and `freight_cost`. Profit is always calculated as `人民币实收 − 采购金额 − 运费`; margin is shown only when RMB received is non-zero, and realized exchange rate is derived only for non-CNY orders. A quotation can be linked to at most one order. Admin can manage all orders, Sales are limited to their customer/opportunity/order scope, and Viewer is read-only. The customer, won-opportunity, and quotation detail pages provide safe order entry points. Business Analytics reports orders by `order_date`, keeps original-currency amounts separate, and aggregates profit inputs only in RMB. Apply migration `0023_add_order_management` after its preceding security migration; it creates the `orders` table and does not rewrite existing customer, quotation, opportunity, or product records.

## Order profit analysis

The **订单管理** page provides an Admin-only profit analysis view. It aggregates the existing order ledger on the backend using `order_date` and the `Asia/Shanghai` business date: today, current month, current quarter, rolling six months, current year, and an inclusive custom range. Original order amounts remain separated by currency; only RMB received, procurement cost, freight, profit, and margin are aggregated. An order is **待核算** when any of the three RMB profit inputs is empty, so it is excluded from profit and margin totals rather than treated as zero. The analysis includes a 12-calendar-month RMB profit trend, Top 10 customer profit ranking, and paginated profit details. Migration `0024_order_profit_pending_accounting` makes these three inputs nullable for future orders while preserving existing historical zero values unchanged.
...
today, yesterday, week, month, year, and inclusive custom-date ranges using the
