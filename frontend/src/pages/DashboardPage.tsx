...
export function DashboardPage() {
  ...
  return <>
    ...
    <section className="mt-6 rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><h3 className="font-bold">客户状态分布</h3><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{stats?.pipeline.map((item) => <div key={item.status} className="flex justify-between rounded-lg bg-slate-50 p-3 text-sm"><span>{customerLabels[item.status] ?? item.status}</span><strong>{item.count}</strong></div>)}</div></section>
  </>;
}
