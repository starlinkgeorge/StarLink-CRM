...
export function MailCenterPage() {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      void getMailMessage(selectedMessageId).then(setSelected).catch(() => undefined);
    }, 20_000);
  ...
  return <>
    ...
    <section className="grid h-[calc(100vh-10rem)] min-h-[38rem] overflow-hidden rounded-xl border bg-white lg:grid-cols-[10rem_minmax(16rem,0.8fr)_minmax(0,1.8fr)]">
      ...
      <section className="flex min-h-0 min-w-0 flex-col border-r"><div className="space-y-2 border-b p-3"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索邮件" className="w-full rounded border px-3 py-2 text-sm" /><select value={customerId} onChange={(event) => setCustomerId(event.target.value)} className="w-full rounded border px-2 py-2 text-sm"><option value="">全部客户</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.company_name}</option>)}</select></div><div className="min-h-0 flex-1 overflow-y-auto">{items.map((message) => <button key={message.id} type="button" onClick={() => void open(message)} className={`flex w-full gap-2 border-b px-3 py-3 text-left hover:bg-slate-50 ${!message.is_read ? "border-l-4 border-l-blue-600 bg-blue-50/60" : ""}`}><span className="text-amber-400">{message.is_starred ? "★" : "☆"}</span><span className="min-w-0 flex-1"><span className="flex justify-between gap-2"><b className="truncate text-sm">{display(message)}</b><time className="text-xs text-slate-500">{date(message.sent_at)}</time></span><span className="block truncate text-sm">{message.subject || "(无主题)"}{message.has_attachments && "  📎"}</span><span className="block truncate text-xs text-slate-500">{message.body_text}</span><TrackingStatus message={message} /></span></button>)}</div></section>
      <section className="min-w-0 overflow-y-auto p-5">{selected ? <><div className="flex justify-between gap-3 border-b pb-4"><div className="min-w-0"><h3 className="break-words text-xl font-bold">{selected.subject || "(无主题)"}</h3><p className="mt-2 break-words text-sm">发件人：{selected.from_name || selected.from_email} &lt;{selected.from_email}&gt;</p><p className="break-words text-sm">收件人：{selected.to_display.join(", ") || selected.to_emails.join(", ")}</p>{selected.cc_emails.length > 0 && <p className="break-words text-sm">抄送：{selected.cc_emails.join(", ")}</p>}<p className="text-xs text-slate-500">{date(selected.sent_at)}</p>{selected.customer_id && <Link to={`/customers/${selected.customer_id}`} className="text-sm text-blue-700">查看关联客户 →</Link>}<TrackingStatus message={selected} detailed /></div><div className="flex h-fit shrink-0 flex-wrap justify-end gap-2 text-sm"><button type="button" onClick={openReaderWindow} className="rounded border px-2 py-1 text-blue-700">新窗口阅读</button><button type="button" onClick={() => void bulkUpdateMail([selected.id], { is_starred: !selected.is_starred }).then(load)}>{selected.is_starred ? "★" : "☆"}</button><button type="button" onClick={() => void bulkUpdateMail([selected.id], { is_read: false }).then(load)}>标记未读</button>{selected.is_draft ? <button type="button" onClick={editDraft}>编辑草稿</button> : <><button type="button" onClick={() => void openCompose("reply")}>回复</button><button type="button" onClick={() => void openCompose("all")}>回复全部</button><button type="button" onClick={() => void openCompose("forward")}>转发</button></>}</div></div><article className="mt-5 break-words text-sm leading-7" dangerouslySetInnerHTML={{ __html: selected.html_body || plainToHtml(selected.body_text) }} />{selected.attachments.map((attachment) => <button key={attachment.id} type="button" onClick={() => void downloadMailAttachment(selected.id, attachment.id).then((blob) => window.open(URL.createObjectURL(blob), "_blank"))} className="mt-3 mr-2 rounded border px-2 py-1 text-sm text-blue-700">📎 {attachment.file_name}</button>)}</> : <p className="pt-24 text-center text-sm text-slate-500">选择邮件查看详情。</p>}</section>
    </section>
