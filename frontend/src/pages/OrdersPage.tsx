...
export function OrdersPage() {
  ...
  const [form, setForm] = useState<OrderPayload>({ order_no: "", customer_id: customerId ?? 0, opportunity_id: Number(params.get("opportunity_id")) || undefined, quotation_id: Number(params.get("quotation_id")) || undefined, order_date: today, currency: "USD", order_amount: "", rmb_received_amount: "", purchase_cost: "", freight_cost: "" });
  ...
  return <>
    ...
    {show && <form onSubmit={submit} className="mt-5 grid gap-3 rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200 md:grid-cols-3">
      ...
      <div className="md:col-span-3 rounded bg-amber-50 px-3 py-2 text-sm text-amber-800">人民币实收、采购金额、运费任一留空时，订单会标记为“待核算”，不会计入利润分析。</div>
      <label>人民币实收<input type="number" min="0" step="0.01" value={form.rmb_received_amount ?? ""} onChange={(event) => setForm({ ...form, rmb_received_amount: event.target.value || undefined })} className="mt-1 w-full rounded border px-3 py-2" /></label>
      <label>采购金额<input type="number" min="0" step="0.01" value={form.purchase_cost ?? ""} onChange={(event) => setForm({ ...form, purchase_cost: event.target.value || undefined })} className="mt-1 w-full rounded border px-3 py-2" /></label>
      <label>运费<input type="number" min="0" step="0.01" value={form.freight_cost ?? ""} onChange={(event) => setForm({ ...form, freight_cost: event.target.value || undefined })} className="mt-1 w-full rounded border px-3 py-2" /></label>
    ...
    <div className="mt-5 overflow-x-auto rounded-xl bg-white shadow-sm ring-1 ring-slate-200"><table className="min-w-[1000px] w-full text-left text-sm"><thead className="bg-slate-50 text-slate-500"><tr>{["订单号", "客户", "日期", "金额", "付款", "生产", "出货", "利润", "负责人"].map((label) => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody>{data?.items.map((order) => <tr key={order.id} className="border-t"><td className="px-4 py-3"><Link className="text-blue-700" to={`/orders/${order.id}`}>{order.order_no}</Link></td><td className="px-4 py-3">{order.customer_company}</td><td className="px-4 py-3">{order.order_date}</td><td className="px-4 py-3">{order.currency} {order.order_amount}</td><td className="px-4 py-3">{payment.find(([value]) => value === order.payment_status)?.[1]}</td><td className="px-4 py-3">{production.find(([value]) => value === order.production_status)?.[1]}</td><td className="px-4 py-3">{shipping.find(([value]) => value === order.shipping_status)?.[1]}</td><td className="px-4 py-3">{order.profit_accounting_status === "Pending" ? <span className="rounded bg-amber-100 px-2 py-1 text-amber-800">待核算</span> : `¥${order.profit}`}</td><td className="px-4 py-3">{order.owner_name ?? "—"}</td></tr>)}{data && !data.items.length && <tr><td colSpan={9} className="px-4 py-8 text-center text-slate-500">暂无匹配订单。</td></tr>}</tbody></table></div>
