...
export interface Order { id:number; order_no:string; customer_id:number; opportunity_id:number|null; quotation_id:number|null; order_date:string; currency:string; order_amount:string; owner_id:number|null; created_by_id:number; payment_status:OrderPaymentStatus; production_status:OrderProductionStatus; shipping_status:OrderShippingStatus; expected_delivery_date:string|null; shipped_at:string|null; notes:string|null; rmb_received_amount:string|null; purchase_cost:string|null; freight_cost:string|null; profit:string|null; profit_margin:string|null; realized_exchange_rate:string|null; profit_accounting_status:"Accounted"|"Pending"; customer_company:string; owner_name:string|null; created_at:string; updated_at:string; }
export interface OrderPage { items: Order[]; total:number; limit:number; offset:number; }
export type OrderProfitPeriod = "today" | "month" | "quarter" | "half_year" | "year" | "custom";
export interface OrderProfitCurrencyAmount { currency:string; amount:string; }
export interface OrderProfitSummary { label:string; start_date:string; end_date:string; order_count:number; accounted_order_count:number; pending_order_count:number; order_amounts:OrderProfitCurrencyAmount[]; rmb_received_total:string; purchase_cost_total:string; freight_cost_total:string; profit_total:string; profit_margin:string|null; }
export interface OrderProfitTrendPoint { month:string; order_count:number; accounted_order_count:number; pending_order_count:number; rmb_received_total:string; profit_total:string; }
export interface OrderProfitCustomerRank { customer_id:number; customer_company:string; order_count:number; accounted_order_count:number; pending_order_count:number; rmb_received_total:string; profit_total:string; profit_contribution_percent:string|null; }
export interface OrderProfitDetail { id:number; order_no:string; customer_id:number; customer_company:string; order_date:string; currency:string; order_amount:string; rmb_received_amount:string|null; purchase_cost:string|null; freight_cost:string|null; profit:string|null; profit_margin:string|null; profit_accounting_status:"Accounted"|"Pending"; }
export interface OrderProfitDetailPage { items:OrderProfitDetail[]; total:number; limit:number; offset:number; }
export interface OrderProfitAnalytics { period:OrderProfitPeriod; selected_summary:OrderProfitSummary; period_summaries:OrderProfitSummary[]; monthly_trend:OrderProfitTrendPoint[]; customer_ranking:OrderProfitCustomerRank[]; details:OrderProfitDetailPage; }
export interface WonOrderBackfillCandidate { opportunity_id:number; opportunity_name:string; customer_id:number; customer_company:string; quotation_id:number|null; quotation_number:string|null; order_date:string|null; order_date_source:string|null; reason:string|null; }
export interface WonOrderBackfillPreview { total_won:number; already_ordered:number; eligible_auto_build:number; requires_date_confirmation:number; unbuildable:number; candidates:WonOrderBackfillCandidate[]; }
export interface WonOrderBackfillResult { created:number; already_ordered:number; requires_date_confirmation:number; unbuildable:number; created_orders:Order[]; skipped:WonOrderBackfillCandidate[]; }
