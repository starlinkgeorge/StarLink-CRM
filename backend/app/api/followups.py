...
async def delete_followup(
    ...
    try:
        ...
        await followup_service.delete_followup(session, followup)
...
async def upload_followup_attachment(
    ...
    try:
        ...
        return await followup_service.create_attachment(
            session,
            followup,
...
async def download_followup_attachment(
    followup_id: int,
    attachment_id: int,
    session: Session = Depends(get_db_session),
    current_user: User = Depends(get_current_user),
    ...
    try:
        ...
        return StreamingResponse(
            BytesIO(await followup_service.attachment_bytes(attachment)),
...
async def delete_followup_attachment(
    followup_id: int,
    attachment_id: int,
    session: Session = Depends(get_db_session),
    current_user: User = Depends(get_current_user),
    ...
    try:
        _ensure_followup_write_access(session, followup_id, current_user)
        attachment = followup_service.get_attachment(session, followup_id, attachment_id)
        await followup_service.delete_attachment(session, attachment)
        return Response(status_code=status.HTTP_204_NO_CONTENT)
    except (NotFoundError, ForbiddenError, StorageConfigurationError) as error:
        _raise_http_error(error)
