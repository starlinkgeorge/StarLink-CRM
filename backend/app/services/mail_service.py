...
def _safe_html(value: str | None) -> str:
    ...
    allowed = r"(?:html|body|a|b|strong|i|em|u|font|span|div|p|br|ul|ol|li|blockquote|h[1-6]|table|thead|tbody|tr|td|th)"
...
def _tracking_pixel_markup(pixel_url: str) -> str:
    """Return the known-good a264 tracking image markup.

    This intentionally stays separate from rich-text sanitization: the URL is
    generated by CRM after sanitization has completed, and is only inserted
    into the final HTML MIME alternative handed to SMTP.
    """
    escaped_url = html.escape(pixel_url, quote=True)
    return (
        f"<img src=\"{escaped_url}\" width=\"1\" height=\"1\" alt=\"\" "
        "style=\"display:none!important;width:1px;height:1px;border:0\" />"
    )


def _html_with_tracking_pixel(body: str, pixel_url: str) -> str:
    """Build the a264 plain-text tracking document for backwards compatibility."""
    escaped_body = html.escape(body or "").replace("\n", "<br>\n")
    return _inject_tracking_pixel_into_html(escaped_body, pixel_url)


def _inject_tracking_pixel_into_html(body_html: str, pixel_url: str) -> str:
    """Inject the a264 pixel into the final legal HTML document sent by SMTP.

    Rich-text editors normally provide an HTML fragment.  A complete document
    is also accepted and receives the pixel immediately before its closing
    body tag, never after ``</html>``.
    """
    content = (body_html or "").strip()
    pixel = _tracking_pixel_markup(pixel_url)
    closing_body = re.compile(r"</body\s*>", re.IGNORECASE)
    if closing_body.search(content):
        return closing_body.sub(lambda match: f"{pixel}{match.group(0)}", content, count=1)
    if re.search(r"<html(?:\s[^>]*)?>", content, re.IGNORECASE):
        # A malformed document without a body still becomes a valid body-based
        # HTML alternative without placing the pixel outside the document.
        return f"<!doctype html><html><body>{content}{pixel}</body></html>"
    return f"<!doctype html><html><body>{content}{pixel}</body></html>"
...
async def send_message(session: Session, user: User, *, recipients: list[str], cc_recipients: list[str], bcc_recipients: list[str], subject: str, body: str, html_body: str, customer_id: int | None, reply_to_id: int | None, forward_of_id: int | None, draft_id: int | None, attachments: list[tuple[str, str | None, bytes]], tracking_enabled: bool = True) -> StoredEmailMessage:
    ...
    if safe_html or tracking_token is not None:
        ...
        if tracking_token is not None:
            # Restore the a264 pixel representation only at final MIME build
            # time.  It must not be persisted as CRM body HTML or sanitized
            # after injection.
            content = _inject_tracking_pixel_into_html(
                content, _tracking_pixel_url(settings, tracking_token)
            )
        else:
            content = f"<!doctype html><html><body>{content}</body></html>"
        outbound.add_alternative(content, subtype="html")
